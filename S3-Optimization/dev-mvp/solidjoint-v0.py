...
# Quantity of liquid to add or remove in the bioreactor to achieve solid loading
model.F_liquid_conv = pe.Var(model.P, model.K_conv, model.X, model.T, domain=pe.Reals)
...
model.Y_conv_f = pe.Var(model.C, model.F, model.P, model.K_conv, model.X, model.J, model.T, domain = pe.Binary)
        ...
        for q in model.Q:
            ...
            for k in model.K_conv:
                if model.Technology_Flow[k] == "IN":
                    ...
                    model.bigM_pathways.add(sum(model.F_out_conv[c, p, k, x, j, t] for c in model.C for p in model.P for j in model.J) <= q * model.Technology_BaseScale[k] + M * (1 - model.Y_tech[k, q, x, t]))
            for k in model.K_pur:
                if model.Technology_Flow[k] == "IN":
                    ...
                    for j in model.J:
                        for f in model.F:
                            ...
                            model.bigM_pathways.add(
                                ...
                                sum(model.Compatibility_Conversion[c, k_pret, k_conv, j, p] * model.F_out_pret[c, f, k_pret, x, t] for k_pret in model.K_pret) 
                                + M * (1 - model.Y_conv_f[c, f, p, k_conv, x, j, t]))
                    
                            # Corresponding to Y_conv[c, p, k_conv, j, t] = False in disjunctive formulation
                            model.bigM_pathways.add(
                                model.F_in_conv[c, f, p, k_conv, x, j, t] <= M * model.Y_conv_f[c, f, p, k_conv, x, j, t])
                        model.bigM_pathways.add(model.Y_conv[c, p, k_conv, x, j, t] <= sum(model.Y_conv_f[c, f, p, k_conv, x, j, t] for f in model.F))
...
# Dont separate solid components from each other or liquid components from each other:
def constraint_solids_liquids(model):
    model.solids_liquids = pe.ConstraintList()
    for t in model.T:
        for x in model.X:
            for f in model.F:
                for k_pret in model.K_pret:
                    for k_conv in model.K_conv:
                        for p in model.P:
                            for j in model.J:
                                c1 = model.C_Solid.at(1)
                                for c in range(2, len(model.C_Solid) + 1):
                                    c2 = model.C_Solid.at(c)
                                    model.solids_liquids.add(model.F_in_conv[c1, f, p, k_conv, x, j, t] * (model.Feedstock_Composition[c2, f] * model.Yield_Pretreatment[c2, k_pret]) 
                                        - (model.Feedstock_Composition[c1, f] * model.Yield_Pretreatment[c1, k_pret]) * model.F_in_conv[c2, f, p, k_conv, x, j, t] 
                                        <= (1 - model.Y_pret[f, k_pret, x, t]) * M
                                    )
                                    model.solids_liquids.add(model.F_in_conv[c1, f, p, k_conv, x, j, t] * (model.Feedstock_Composition[c2, f] * model.Yield_Pretreatment[c2, k_pret]) 
                                        - (model.Feedstock_Composition[c1, f] * model.Yield_Pretreatment[c1, k_pret]) * model.F_in_conv[c2, f, p, k_conv, x, j, t] 
                                        >= - (1 - model.Y_pret[f, k_pret, x, t]) * M
                                    )
                                c1 = model.C_Liquid.at(1)
                                for c in range(2, len(model.C_Liquid) + 1):
                                    c2 = model.C_Liquid.at(c)
                                    model.solids_liquids.add(model.F_in_conv[c1, f, p, k_conv, x, j, t] * (model.Feedstock_Composition[c2, f] * model.Yield_Pretreatment[c2, k_pret]) 
                                        - (model.Feedstock_Composition[c1, f] * model.Yield_Pretreatment[c1, k_pret]) * model.F_in_conv[c2, f, p, k_conv, x, j, t] 
                                        <= (1 - model.Y_pret[f, k_pret, x, t]) * M
                                    )
...
#constraint_solids_liquids(model)


model.Y_conv_pret_feed = pe.Var(model.C, model.F, model.P, model.K_pret, model.K_conv, model.X, model.J, model.T, domain = pe.Binary)
# Dont separate solid components from each other or liquid components from each other:
def constraint_solids_liquids(model):
    model.solids_liquids = pe.ConstraintList()
    for t in model.T:
                        ...
                        for p in model.P:
                            for j in model.J:
                                ...
                                for c in model.C_Solid:
                                    model.solids_liquids.add(model.Y_conv_pret_feed[c, f, p, k_pret, k_conv, x, j, t] >= model.Y_pret[f, k_pret, x, t] + model.Y_conv_feed[c, f, p, k_conv, x, j, t] - 1)
                                    model.solids_liquids.add(model.Y_conv_pret_feed[c, f, p, k_pret, k_conv, x, j, t] <= model.Y_conv_feed[c, f, p, k_conv, x, j, t])
                                    model.solids_liquids.add(model.Y_conv_pret_feed[c, f, p, k_pret, k_conv, x, j, t] <= model.Y_pret[f, k_pret, x, t])   
                                    for c2 in model.C_Solid:
                                        if c2 != c:
                                            c_list.append(c2)
                                            model.solids_liquids.add(
                                                model.F_in_conv_feed[c, f, p, k_conv, x, j, t] * (model.Feedstock_Composition[c2, f] * model.Yield_Pretreatment[c2, k_pret]) 
                                                - (model.Feedstock_Composition[c, f] * model.Yield_Pretreatment[c, k_pret]) * model.F_in_conv_feed[c2, f, p, k_conv, x, j, t] 
                                                <= (1 - model.Y_conv_pret_feed[c, f, p, k_pret, k_conv, x, j, t]) * M
                                            )
                        ...
                        for p in model.P:
                            for j in model.J:
                                ...
                                for c in range(2, len(model.C_Solid) + 1):
                                    ...
                                    c_list.append(c2)
                                    model.solids_liquids.add(
                                        model.F_in_conv_feed[c1, f, p, k_conv, x, j, t] * (model.Feedstock_Composition[c2, f] * model.Yield_Pretreatment[c2, k_pret]) 
                                        - (model.Feedstock_Composition[c1, f] * model.Yield_Pretreatment[c1, k_pret]) * model.F_in_conv_feed[c2, f, p, k_conv, x, j, t] 
                                        <= (1 - model.Y_pret[f, k_pret, x, t]) * M
                                    )
                                    model.solids_liquids.add(model.F_in_conv_feed[c1, f, p, k_conv, x, j, t] * (model.Feedstock_Composition[c2, f] * model.Yield_Pretreatment[c2, k_pret]) 
                                        - (model.Feedstock_Composition[c1, f] * model.Yield_Pretreatment[c1, k_pret]) * model.F_in_conv_feed[c2, f, p, k_conv, x, j, t] 
                                        >= - (1 - model.Y_pret[f, k_pret, x, t]) * M
                                    )
                                    model.solids_liquids.add(
                                        model.F_in_conv_feed[c1, f, p, k_conv, x, j, t] * (model.Feedstock_Composition[c2, f] * model.Yield_Pretreatment[c2, k_pret]) 
                                        - (model.Feedstock_Composition[c1, f] * model.Yield_Pretreatment[c1, k_pret]) * model.F_in_conv_feed[c2, f, p, k_conv, x, j, t] 
                                        <= (1 - model.Y_conv_feed[c1, f, p, k_conv, x, j, t]) * M
                                    )
                                    model.solids_liquids.add(model.F_in_conv_feed[c1, f, p, k_conv, x, j, t] * (model.Feedstock_Composition[c2, f] * model.Yield_Pretreatment[c2, k_pret]) 
                                        - (model.Feedstock_Composition[c1, f] * model.Yield_Pretreatment[c1, k_pret]) * model.F_in_conv_feed[c2, f, p, k_conv, x, j, t] 
                                        >= - (1 - model.Y_conv_feed[c1, f, p, k_conv, x, j, t]) * M
                                    )
                                #model.solids_liquids.add(model.Y_conv_feed[c1, f, p, k_conv, x, j, t] <= sum(model.Y_conv_feed[c2, f, p, k_conv, x, j, t] for c2 in c_list ))
...
def constraint_sl_liquid(model):
    ...
    for t in model.T:
        for x in model.X:
            for p in model.P:
                for k_conv in model.K_conv:
                    if k_conv in ['SHF', 'SSF']:
                        model.sl_liquid.add(model.F_liquid_conv[p, k_conv, x, t] == 
                                            ((1 - model.Solid_Loading[k_conv]) / model.Solid_Loading[k_conv]) * sum(model.F_in_conv[c, f, p, k_conv, x, j, t] for c in model.C_Solid for f in model.F for j in model.J)
                                            )
                    if k_conv in ['SHCF', 'SSCF']:
                        #model.sl_liquid.add(sum(model.F_in_conv[c, p, k_conv, x, j, t]  for c in model.C_Liquid for j in model.J) >= 
                         #                       ((1 - model.Solid_Loading[k_conv]) / model.Solid_Loading[k_conv]) * sum(model.F_in_conv[c, p, k_conv, x, j, t] for c in model.C_Solid for j in model.J)
                          #                      - M * (1 - sum(model.Y_conv[c, p, k_conv, x, j, t] for c in model.C for j in model.J))
                            #                    )
                        model.sl_liquid.add(model.F_liquid_conv[p, k_conv, x, t] == 
                                            ((1 - model.Solid_Loading[k_conv]) / model.Solid_Loading[k_conv]) * sum(model.F_in_conv[c, f, p, k_conv, x, j, t] for c in model.C_Solid for f in model.F for j in model.J) 
                                            - sum(model.F_in_conv[c, f, p, k_conv, x, j, t]  for c in model.C_Liquid for f in model.F for j in model.J)
                                            )
                    else:
                        model.sl_liquid.add(model.F_liquid_conv[p, k_conv, x, t] == 0)
...
for t in model.T:
    ...
    for x in model.X:
        ...
        for c in model.C:
            model.Mass_Balances.add(sum(model.F_in_conv[c, f, p, k, x, j, t] for f in model.F for p in model.P for k in model.K_conv for j in model.J) <= sum(model.F_out_pret[c, f, k, x, t] for f in model.F for k in model.K_pret))     
        ...
        for f in model.F:
            ...
            for k_pret in model.K_pret:
                ...
                #model.Mass_Balances.add(sum(model.F_out_pret[c, f, k_pret, x, t] for c in model.C) == model.F_in_pret[f, k_pret, x, t])

        for p in model.P:
            for k_conv in model.K_conv:
                for c in model.C:
                    for j in model.J:
                        model.Mass_Balances.add(model.F_out_conv[c, p, k_conv, x, j, t] == sum(model.F_in_conv[c, f, p, k_conv, x, j, t] for f in model.F) * model.Yield_Conversion[p, c, k_conv, j])

            # only one kconv per product
            model.Mass_Balances.add(sum(model.Y_conv[c, p, k_conv, x, j, t] for c in model.C for j in model.J for k_conv in model.K_conv) <= 1)
        ...
        for u in model.U:
            ...
            for k in model.K_pret:
                if model.Utility_Consumption[u, k] != "Enzymes":
                    ...
                    model.utility_consumption.add(model.F_util[u, k, x, t] == sum(model.F_out_pret[c, f, k, x, t] for c in model.C for f in model.F) * model.Utility_Consumption[u, k])
            for k in model.K_conv:
                    model.utility_consumption.add(model.F_util[u, k, x, t] == sum(model.F_in_conv[c, f, p, k, x, j, t] for c in model.C for p in model.P for j in model.J)  * model.Utility_Consumption[u, k])
            for k in model.K_pur:
...
for t in model.T:
    for x in model.X:
        for bp in model.BP:
            ...
            for k in model.K_conv:
                    model.byproduct_generation.add(model.F_bypr[bp, k, x, t] == sum(model.F_in_conv[c, f, p, k, x, j, t] for c in model.C for p in model.P for j in model.J)  * model.Byproduct_Production[bp, k])
            for k in model.K_pur:
...
model.Depreciation = pe.Var(model.X, domain = pe.NonNegativeReals)
model.Depreciation_calc = pe.Constraint(model.X,
                                        rule = lambda m, x: m.Depreciation[x] == sum(((max(m.T) - t + 1) * m.DepreciatonFactor * m.CAPEX[x, t]) for t in model.T))
...
def NPV_rule(m):      
    return sum( sum((m.EBIT_Plant[x, t] - m.CAPEX[x, t]) for t in model.T) + m.Depreciation[x] for x in m.X)

model.efactor = pe.Var(domain = pe.Reals)
model.efactor_constraint = pe.Constraint(rule =
                                          model.efactor * sum(model.F_out_prod[mk, p, x, t] for mk in model.M for p in model.P for x in model.X for t in model.T) 
                                          == sum(model.F_bypr[bp, k, x, t] for bp in model.BP for k in model.K for x in model.X for t in model.T))

def efactor_rule(m):
    return model.efactor
