...
def create_sets(sets = {}, new = {}, default = True, jsonfile = None):
    ...
    if default:
        sets = {
            ...
            "Q"       : [0.5, 1, 1.5, 1.75, 1.9, 2, 3, 4, 5, 6, 7, 8, 9, 10, 50,  100]  # Indices for technology capacity levels
        }
        
        for key, values in new.items():
            sets[key] = sets[key] + values
...
def create_parametric_dic(sets, compat_dfs):
    ...
    dfs['Utility_Consumption'] = create_parametric_df({'U': sets['U'], 'K': sets['K']}, cols = ["Flow"])
    ...
    dfs['Compatibility_Conversion_Full'] = df.copy()
    dfs['Compatibility_Conversion'] = df[['K_pret', 'K_conv', 'J', 'Value']].drop_duplicates(['K_pret', 'K_conv', 'J'], keep="first").reset_index(drop=True)
...
def read_parametric_excel(sets, xls_path, compat = None):
    ...
    for name, df in dfs.items():
        ...
        if name == "Technology_Cost":
            ...
            df.Flow = df.Flow.fillna("IN")
        if name == "Utility_Consumption":
            df = df.drop_duplicates(["U", "K"]).reset_index(drop=True)
            df.Flow = df.Flow.fillna("IN")
        # change all NA to 0 excpet for solid loading
        df.Value = df.Value if name in ["Solid_Loading"] else df.Value.fillna(0)
...
def import_paremeters(sets, fp = "Inputs\inputs.xlsx"):
